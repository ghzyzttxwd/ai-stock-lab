...
def _index_features(ak, trade_date: str) -> dict:
    ...
    for symbol in specs:
        ...
        per.append({
            'symbol': symbol, 'r3': round(r3, 6), 'r20': round(r20, 6),
            'r60': round(r60, 6), 'drawdown20': round(dd20, 6),
...
def _sentiment_snapshot(ak, trade_date: str) -> dict:
    ...
    for _, r in up.iterrows():
        code = _raw_code(r.get('代码'))
        if not _is_mainboard(code):
            continue
        main_up.append({
            'code': code, 'name': _clean_text(r.get('名称')),
            'boards': int(_num(r.get('连板数'), 1) or 1),
            'breaks': int(_num(r.get('炸板次数'), 0) or 0),
            'short_industry': _clean_text(r.get('所属行业')),
            'first_seal': _clean_text(r.get('首次封板时间')),
            'last_seal': _clean_text(r.get('最后封板时间')),
        })
    ...
    for _, r in down.iterrows():
        code = _raw_code(r.get('代码'))
        if _is_mainboard(code):
            main_down.append({'code': code, 'name': _clean_text(r.get('名称'))})
...
def _preselect_style_union(
    ...
    pools = {}
    pools['A'] = top(
        lambda x: (x.get('quality_score') or -1, x.get('amount', 0)), 45,
    ...
    for label, pool in pools.items():
        for x in pool:
            code = x['raw_code']
            chosen[code] = x
            membership[code].add(label)
    if len(chosen) > max_union:
        ordered = sorted(
            chosen.values(),
            key=lambda x: (len(membership[x['raw_code']]), x.get('amount', 0)),
            reverse=True,
        )
...
def build_snapshot(requested_date: str) -> dict:
    ...
    if reported_date != trade_date:
        raise RuntimeError(
            f'forward snapshot refuses stale/future market activity: reported={reported_date} trade_date={trade_date}'
        )
    ...
    return {
        ...
        'source_notes': {
            ...
            'industry': 'Shenwan L1',
            'fundamentals': 'Eastmoney performance report; mainboard only; announcement-date filtered; one common scoring period',
        ...
        'market': {
            'regime': {
                ...
                'confidence': regime.confidence, 'reasons': list(regime.reasons),
            },
            'features': regime_features,
            'activity': {
                'up': up, 'down': down, 'flat': flat, 'active': active,
                'advancer_ratio': round(up / active, 4), 'reported_date': reported_date,
            },
            'high_low': highlow,
            'sentiment': {
                k: v for k, v in sentiment.items()
                if k not in {'limit_up', 'broken_limit', 'limit_down'}
            },
        },
        'industry': {
            'taxonomy': industry['taxonomy'], 'counts': industry['counts'],
            'top_strength': sorted(
                industry['industries'].values(), key=lambda x: x['industry_score'], reverse=True
            )[:12],
            'bottom_strength': sorted(
                industry['industries'].values(), key=lambda x: x['industry_score']
            )[:8],
        },
        'fundamentals': {
