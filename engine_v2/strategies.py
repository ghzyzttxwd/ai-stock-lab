...
def strategy_a_v2(candidates: list[dict], regime: MarketRegime, fund_drawdown: float = 0.0) -> list[dict]:
    """A: quality + low risk + defensive trend."""
    eligible = [
        x for x in candidates
        if _f(x, "quality_score") >= 55
    ...
    for x in eligible:
        ...
        ranked.append(_decorate(
            x, score,
            "盈利/现金流质量合格、波动可控且价格趋势未明显恶化",
            "财务质量跌破门槛、风险显著上升或中期趋势破坏",
        ))
...
def strategy_d_fallback_v2(candidates: list[dict], regime: MarketRegime, fund_drawdown: float = 0.0) -> list[dict]:
    ...
    for x in candidates:
        ...
        ranked.append(_decorate(
            x, score,
            "技术、行业地位、质量、估值和风险共同支持，作为D的规则兜底候选",
...
def strategy_l_v2(candidates: list[dict], regime: MarketRegime, fund_drawdown: float = 0.0) -> list[dict]:
    """L: quality/value with cash-flow discipline and a trend sanity check."""
    eligible = [
        x for x in candidates
        if _f(x, "quality_score") >= 60
        and _f(x, "cashflow_score") >= 55
        and _f(x, "balance_sheet_score") >= 50
        and _f(x, "trend") >= 35
        and not bool(x.get("financial_distress"))
    ]
    ranked = []
    for x in eligible:
        score = (
            0.30 * _f(x, "quality_score")
            + 0.20 * _f(x, "cashflow_score")
            + 0.20 * _f(x, "valuation_score")
            + 0.12 * _f(x, "balance_sheet_score")
            + 0.08 * _f(x, "trend")
            + 0.06 * _f(x, "risk")
            + 0.04 * _f(x, "industry_score")
        )
        ranked.append(_decorate(
            x, score,
            "盈利质量、经营现金流和资产负债表健康，估值具有行业相对吸引力",
            "盈利/现金流恶化、杠杆异常上升、估值失去安全边际或基本面逻辑改变",
        ))
    ranked.sort(key=lambda x: x["v2_score"], reverse=True)
    exposure = _exposure(regime, {
