...
def _empty_execution(trade_date: str, decision_date: str | None = None, **extra) -> dict:
    return {
        'phase':'conditional_buy','decision_date':decision_date,'trade_date':trade_date,
        'reference_price_field':'conditional_trigger','fills':[],'rejected_orders':[],
        'policy_adjustments':[],'valuation_fallback_symbols':[],'fees':0.0,**extra,
    }
...
def run_shadow_session_split(
    ...
    if not safety.get('ready_for_strategy_targets') or safety.get('calls_sol'):
        raise RuntimeError('V2 conditional shadow accounting blocked by enrichment safety')
    ...
    for fund_id, state in states.items():
        ...
        if pending:
            ...
            if decision_date == previous_trade_date:
                is_new = (
                    pending.get('plan_version') == PLAN_VERSION
                    or pending_is_conditional(pending)
                    or (not (pending.get('targets') or []) and pending.get('execution_model') == EXECUTION_MODEL)
                )
                if is_new:
                    if pending.get('targets'):
                    ...
                    # Migration boundary: a legacy target had no pre-declared price condition,
                    # therefore it is cancelled rather than retroactively inventing a trigger.
                    execution[fund_id] = _empty_execution(
                        trade_date, decision_date,
                        legacy_fixed_price_plan_cancelled=True,
                        cancelled_targets=len(pending.get('targets') or []),
                    )
            ...
            execution[fund_id] = _empty_execution(trade_date)
    ...
    if not target_safety.get('targets_valid') or target_safety.get('calls_sol') or target_safety.get('executes_orders'):
        raise RuntimeError('V2 conditional targets failed safety gate before ledger commit')
    if targets_payload.get('plan_version') != PLAN_VERSION:
        raise RuntimeError('V2 target payload is not the expected conditional-plan version')
    ...
    fund_events = {}
    for fund_id, state in states.items():
        next_targets=list((targets_payload.get('targets') or {}).get(fund_id) or [])
        refresh_rotation_flags(state, next_targets, trade_date)
